...
class PostCategory extends Model
{
    ...
    {
        $category = $this->where('slug', $slug)->first();
        $post = DB::table('posts')->where('post_category_id', $category->id)->count();
        DB::table('posts')->where('post_category_id', $category->id)->update([
            'post_category_id' => 0
        ]);

        $uncategorized = $this->where('id', 0)->first();
        $uncategorized->update(['count' => $uncategorized->count + $post]);
