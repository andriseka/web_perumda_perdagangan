...
class AwardCategory extends Model
{
    ...
    public function storeData(array $data)
    {
        $validation = Validator::make($data, [
            'name' => 'required|string|max:255|unique:award_categories,name,',
    ...
    public function updateData(array $data, $slug)
    {
        ...
        $validation = Validator::make($data, [
            'name' => 'required|string|max:255|unique:award_categories,name,'.$category->id,
            'desc' => 'nullable|string'
        ]);
        if ($validation->fails()) {
            $response = [
    ...
    {
        $category = $this->where('slug', $slug)->first();
        $award = DB::table('awards')->where('award_category_id', $category->id)->count();
        DB::table('awards')->where('award_category_id', $category->id)->update([
            'award_category_id' => 0
        ]);

        $uncategorized = $this->where('id', 0)->first();
        $uncategorized->update(['count' => $uncategorized->count + $award]);
