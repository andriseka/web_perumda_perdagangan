    ...
    <div class="row">
        <div class="col-md-4 mb-3">
            <div class="card">
                <div class="card-body">
                    <div>
                        <h4 class="fw-bolder">Buat RKAP</h4>
                    </div>
                    <form action="{{ url('/admin/rkap/store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Upload Excel RKAP</label>
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <div>
                        <h4 class="fw-bolder">Daftar RKAP</h4>
                    </div>
                    <div>
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach ($data['data'] as $rkap)
                                        <tr>
                                            <td class="text-center">{{ $rkap['no'] }}</td>
                                            <td>{{ $rkap['year'] }}</td>
                                            <td>Rp. {{ number_format($rkap['total_anggaran'], 0 , ',', '.') }}</td>
                                            <td class="text-center">
                                                @if ($rkap['file'] !== null)
                                                    <a href="{{ asset('uploads/rkap/'.$rkap['file']) }}" download="RKAP Tahun {{ $rkap['year'] }}" >download</a>
                                                @else
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center">
                                                    <a href="" class="btn btn-success btn-sm me-2" data-bs-toggle="modal" data-bs-target="#{{ $rkap['year'] }}">Edit</a>
                                                    <form action="{{ url('/admin/rkap/delete', $rkap['year']) }}" method="POST" enctype="multipart/form-data">
                                                        @csrf
                                                        @method('delete')
                                        ...
                                        <div class="modal fade" id="{{ $rkap['year'] }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-sm modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ url('/admin/rkap/update', $rkap['id']) }}" method="POST" enctype="multipart/form-data">
                                                        @csrf
                                                        @method('patch')
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                                                                <label class="form-label">Tahun</label>
                                                                <input type="number" min="2000" name="year" placeholder="Tulis Tahun" class="form-control" value="{{ $rkap['year'] }}" required>
                                                            </div>
                                                            <div class="mb-3">
                                                                <label class="form-label">Total Anggaran</label>
                                                                <input type="text" id="currencyedit" class="form-control" name="total_anggaran" placeholder="Rp. " value="Rp. {{ number_format($rkap['total_anggaran'],0,',','.') }}" required>
                                                            </div>
                                                            <div class="mb-3">
                                                                <label class="form-label">Keterangan</label>
                                                                <textarea name="desc" style="height: 80px" class="form-control" placeholder="Tulis Keterangan">{{ $rkap['desc'] }}</textarea>
                                                            </div>
                                                            <div class="mb-3">
                                                                <label class="form-label">Upload Excel RKAP</label>
                                                                <input type="file" class="form-control" name="file" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
