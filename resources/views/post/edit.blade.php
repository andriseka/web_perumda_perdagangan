...
@section('content')
    <div class="mb-4 d-flex justify-content-between items-conten-center">
        ...
        <div>
            <a href="{{ url('/admin/post') }}">Postingan</a> / {{ $post['title']  }}
        </div>
    </div>

    <form action="{{ url('/admin/post/update', $post['slug']) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('patch')
        <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="title" placeholder="Tulis Judul Postingan" value="{{ $post['title'] }}" required >
                            @error('title')
                    ...
                    <div class="card-body">
                            ...
                            <select name="post_category_id" class="form-select">
                                @foreach ($categories as $category)
                                    <option value="{{ $category['id'] }}"
                                        @if ($category['id'] === (int)$post['post_category_id'])
                                            selected
                                        @endif
                        ...
                        <div class="mb-3">
                            ...
                            <div id="image-preview">
                                @if ($post['image'] !== null)
                                    <img src="{{ $post['image'] }}" class="rounded mt-2" style="width: 100%; height: 180px; box-shadow: 1px 1px 5px #00000038; ">
                                @endif
                            </div>
                        </div>
