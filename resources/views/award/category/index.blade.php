...
@section('content')
    <div class="mb-4">
        <h3 class="fw-bolder">KATEGORI PENGHARGAAN</h3>
    </div>
    ...
    <div class="row">
        <div class="col-md-4 mb-3">
            <div class="card">
                <div class="card-body">
                    <div>
                        <h4 class="fw-bolder">Buat Kategori Penghargaan</h4>
                    </div>

                    <form action="{{ url('/admin/award/category/store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label">Nama Kategori</label>
                            <input type="text" class="form-control" placeholder="Tulis Nama Kategori" name="name" value="{{ old('name') }}" required>
                            @error('name')
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <div>
                        <h4 class="fw-bolder">Daftar Kategori Penghargaan</h4>
                    </div>
                            ...
                            <tbody>
                                @foreach ($data['data'] as $category)
                                    <tr>
                                        ...
                                        <td class="text-center">{{ $category['count'] }} Posting</td>
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                @if ($category['slug'] !== 'uncategorized')
                                                    <a href="" class="btn btn-success btn-sm me-2" data-bs-toggle="modal" data-bs-target="#{{ $category['slug'] }}">Edit</a>
                                                    <form action="{{ url('/admin/award/category/delete', $category['slug']) }}" method="POST">
                                                        @csrf
                                                        @method('delete')
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title fw-bolder">
                                                        Edit Kategori Penghargaan
                                                    </h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <form action="{{ url('/admin/award/category/update', $category['slug']) }}" method="POST">
                                                    @csrf
                                                    @method('patch')
